...
for msg in st.session_state.messages:
    if isinstance(msg, dict):  # If stored as dictionary
        role = "user" if msg.get("role") == "user" else "assistant"
        content = msg.get("content", "")

        # Skip invalid messages
        if content is None or content == "INVALID_PLAYERS" or isinstance(content, dict):  
            continue

    else:  # If stored as ChatCompletionMessage object
...
#         # Skip invalid or tool-call messages
#         if msg.get("role") == "tool" and msg.get("content") == "INVALID_PLAYERS":
#             continue
#         if isinstance(msg.get("content"), json):
#             continue
